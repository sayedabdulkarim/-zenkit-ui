    ...
    const defaultHeader = (
      <div className="calendar-header">
        <div className="calendar-header-nav">
          <button type="button" className="calendar-nav-btn" onClick={() => navigateMonth(-1)}>
            <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none">
              <polyline points="15 18 9 12 15 6" />
            </svg>
          </button>
        </div>
        <button type="button" className="calendar-title" onClick={() => handleModeChange(currentMode === 'month' ? 'year' : 'month')}>
          {currentMode === 'month'
              ...
              {getDaysInMonth().map(({ date, isCurrentMonth }, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {dateCellRender && <div className="calendar-day-events">{dateCellRender(date)}</div>}
                  </div>
                );
              })}
            </div>
            ...
            {MONTHS.map((month, i) => (
              <div
                ...
                className={cn(
                  ...
                  currentDate.getMonth() === i && 'is-selected'
                )}
                onClick={() => {
                  const newDate = new Date(currentDate);
                  newDate.setMonth(i);
